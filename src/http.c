        ...
        { /* Connect */
            ...
            { /* Compose request */
                ...
                snprintf(
                    ...
                    !strcmp(port, "http") ? "" : ":", !strcmp(port, "http") ? "" : port, VERSION, ifrange[0] ? ifrange : "", authhdr ? authhdr : ""
                );
            ...
            { /* Authorization required */
                ...
                if (authhdr)
                { /* Go around again with auth header */
                    fclose(f);
                    f = NULL;
                }
                else
                { /* No auth details available for this host - error out */
                    fclose(f);
                    f = NULL;
                    break;
                }
            ...
            { // Precondition (i.e. if-unmodified-since) failed
                ifrange[0] = 0;
                fclose(f);
            ...
            else if (code == 206 && fname)
            { // Had partial content and server confirms not modified
                /* Append to existing on-disk content (plus allow reading once we finish) */
                g = fopen(fname, "a+");
            }
            else if (code == 304)
            { // Unchanged (if-modified-since was false)
...
struct range_fetch
{
    ...
    int server_close; /* 0: can send more, 1: cannot send more (but one set of headers still to read), 2: cannot send more and all existing headers
                         read */
...
static void range_fetch_getmore(struct range_fetch* rf)
{
    ...
    snprintf(
        ...
        "%s"
        "Range: bytes=",
        rf->url, rf->hosth, referer ? "\r\nReferer: " : "", referer ? referer : "", rf->authh ? rf->authh : ""
    );
    ...
    for (; rf->rangessent < rf->nranges;)
    {
        ...
        snprintf(
            request + l, sizeof(request) - l, OFF_T_PF "-" OFF_T_PF "%s", rf->ranges_todo[2 * i], rf->ranges_todo[2 * i + 1], lastrange ? "" : ","
        );
...
int range_fetch_read_http_headers(struct range_fetch* rf)
{
    ...
    while (1)
    {
        ...
        if (buf[0] == '\r' || buf[0] == '\0')
        {
            ...
            if (((rf->boundary || rf->block_left) && !(rf->boundary && rf->block_left)) || (status >= 300 && status < 400 && seen_location))
                return status;
            break;
        }
        ...
        *p = 0;
        ...
        if (status == 206 && !strcasecmp(buf, "content-type") && !strncasecmp(p, "multipart/byteranges", 20))
        {
        ...
        if ((status == 302 || status == 301) && !strcmp(buf, "location"))
        {
            if (seen_location++)
            {
