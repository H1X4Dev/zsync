...
struct rcksum_state* rcksum_init(zs_blockid nblocks, size_t blocksize, int rsum_butes, unsigned int checksum_bytes, int require_consecutive_matches);
void                 rcksum_end(struct rcksum_state* z);

/* These transfer out the filename and handle of the file backing the data retrieved.
 * Once you have transferred out the file handle, you can no longer read and write data through librcksum - it has handed it over to you, and can use
 * it no more itself. If you transfer out the filename, you are responsible for renaming it to something useful. If you don't transfer out the
 * filename, librcksum will unlink it at rcksum_end.
 */
