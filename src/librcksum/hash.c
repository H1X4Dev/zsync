...
static void print_hashstats(const struct rcksum_state* z)
{
    ...
    int i;
    {
        ...
        fprintf(stderr, "bithash %dKB, density %.1f%%\n", (z->bithashmask + 1) / (1000 * 8), 100.0 * num_bits_set / (z->bithashmask + 1));
    }
    {
        ...
        fprintf(
            stderr, "rsum hash density: %d/%d %.1f%% (depth avg: %.1f, max: %d)\n", hash_entries_used, z->hashmask + 1,
            100.0 * hash_entries_used / (z->hashmask + 1), z->blocks / (float)hash_entries_used, max_depth
        );
    }
    #if 0
...
{
    sprintf(
        buf, "%x%x%x%x%x%x%x%x%x%x%x%x%x%x%x%x", t->checksum[0], t->checksum[1], t->checksum[2], t->checksum[3], t->checksum[4], t->checksum[5],
        t->checksum[6], t->checksum[7], t->checksum[8], t->checksum[9], t->checksum[10], t->checksum[11], t->checksum[12], t->checksum[13],
