 ...
 */
struct rcksum_state* rcksum_init(zs_blockid nblocks, size_t blocksize, int rsum_bytes, unsigned int checksum_bytes, int require_consecutive_matches)
{
...
#ifdef DEBUG
    fprintf(
        stderr, "hashhit %lld, weakhit %d, checksummed %d, stronghit %d\n", z->stats.hashhit, z->stats.weakhit, z->stats.checksummed,
        z->stats.stronghit
    );
#endif
    free(z);
