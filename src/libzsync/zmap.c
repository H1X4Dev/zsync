 ...
 */
static int
find_compressed_ranges_for(const struct zmap* zm, off_t* zbyterange, int k, long long* lastwroteblockstart_inbitoffset, long long start, long long end)
{
    int j;
...
off_t* zmap_to_compressed_ranges(const struct zmap* zm, off_t* byterange, int nrange, int* num)
{
    ...
    for (i = 0; i < nrange; i++)
    {
         ...
         * byterange. */
        k = find_compressed_ranges_for(zm, zbyterange, k, &lastwroteblockstart_inbitoffset, byterange[2 * i], byterange[2 * i + 1]);
        if (k < 0)
        {
