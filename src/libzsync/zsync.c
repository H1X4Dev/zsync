...
static const char* const gzip_safe_option[] = { "--best",           "",          "--rsync",           "--rsync --best",
                                                "--best --no-name", "--no-name", "--rsync --no-name", "--rsync --best --no-name" };
static const int         gzip_safe_options  = sizeof(gzip_safe_option) / sizeof *gzip_safe_option;
...
static int    zsync_read_blocksums(struct zsync_state* zs, FILE* f, int rsum_bytes, int checksum_bytes, int seq_matches);
    ...
    for (;;)
    {
        ...
        if (p && *(p + 1) == ' ')
        {
            *p++ = 0;
            ...
            {
                if (sscanf(p, "%d,%d,%d", &seq_matches, &rsum_bytes, &checksum_bytes) != 3 || rsum_bytes < 1 || rsum_bytes > 4 || checksum_bytes < 3 ||
                    checksum_bytes > 16 || seq_matches > 2 || seq_matches < 1)
                {
...
static int zsync_read_blocksums(struct zsync_state* zs, FILE* f, int rsum_bytes, unsigned int checksum_bytes, int seq_matches)
{
...
off_t* zsync_needed_byte_ranges(struct zsync_state* zs, int* num, int type)
{
    ...
    case 0:
        *num = nrange;
        return byterange;
    case 1: { /* Convert ranges in the uncompressed data to ranges in the compressed data */
 ...
 */
static void zsync_configure_zstream_for_zdata(const struct zsync_state* zs, struct z_stream_s* zstrm, long zoffset, off_t* poutoffset)
{
 ...
 */
static int zsync_receive_data_uncompressed(struct zsync_receiver* zr, const unsigned char* buf, off_t offset, size_t len)
{
    int    ret       = 0;
 ...
 * expected checksum for the corresponding blocks)
 */
static int zsync_receive_data_compressed(struct zsync_receiver* zr, const unsigned char* buf, off_t offset, size_t len)
{
    int    ret       = 0;
    int    eoz       = 0;
    size_t blocksize = zr->zs->blocksize;
    ...
    if (zr->strm.total_in == 0 || offset != zr->strm.total_in)
    {
        ...
        /* On first iteration, we might be reading an incomplete block from zsync's point of view. Limit avail_out so we can stop after doing that and
         * realign with the buffer. */
    ...
    else
    {
        if (zr->outoffset == -1)
        {
            fprintf(stderr, "data didn't align with block boundary in compressed stream\n");
            return 1;
        }
        zr->strm.next_in  = buf;
        zr->strm.avail_in = len;
