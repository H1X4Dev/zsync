...
int fetch_remaining_blocks_http(struct zsync_state* z, const char* u, int type)
{
    int                    ret = 0;
 ...
 */
int fetch_remaining_blocks_from_url(struct zsync_state* zs, const char* url, int type)
{
    ...
    /* STEP 3: fetch remaining blocks via the URLs from the .zsync */
    {
        ...
        if (target_status < 2)
        {
            fprintf(
                ...
                fetch_status == 0 ? "No download URLs are known, so no data could be downloaded. The .zsync file is probably incomplete."
                : target_status == 0
